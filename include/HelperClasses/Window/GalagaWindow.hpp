...
namespace DisplayModule
{
    ...
    enum PathNodeType {
        FLY_THRU,
        FLY_TO, 
        TURN_LEFT, //turns solve for the tangent of the path to the next node
                   //the unit will turn around turn_radius to try and line up with the straight line path to the next node
        TURN_RIGHT,
        ROTATE_TO, //rotates in place to desired orientation
    };
    ...
    uint8_t orientation_code_from_orientation_degrees_100(uint32_t orientation_degrees_100) {
    ...
    class PathNode {
        ...
        uint32_t turn_radius_100; //100X the turn radius in pixels of this node. Does nothing if this isn't a turn!

        uint32_t target_pos_x_100; //100x the X coord of the FLY_THRU or FLY_TO value. Where it'll try to go. 
        uint32_t target_pos_y_100; //         Y                           

        uint32_t orentation_target_degrees_100; //target in degrees to rotate to x100
    ...
    //pathfinding defs
    enum PathType {
        FLY_THRU, //flies straight through a point it reaches. Keeps going. 
        FLY_TO,   //flies to a point. Stays put
    }
